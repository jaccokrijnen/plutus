...
data PassId
  ...
  -- Simplifier
  | PassInline -- [PIR.Name] [PIR.TyName]
               -- The Names that were unconditionally inlined and thus eliminated
  | PassUnwrapWrap
  | PassCaseReduce
  | PassCaseOfCase
  | PassBeta
...
class Meta a m where
  writeMeta :: a -> m ()

newtype RenameMeta = RenameMeta [Unique]

instance MonadIO m => Meta RenameMeta m where
  writeMeta (RenameMeta xs) = liftIO $ print xs

-- | A pass over a term, with pre- and post-conditions.
data Pass m tyname name uni fun a where
  ...
  -- | For passes that produce some additional information
  -- PassMeta :: forall meta m tyname name uni fun a.
  --   Show meta =>
  --   (Pass (WriterT meta m) tyname name uni fun a) ->
  --   Pass m tyname name uni fun a
...
runPass
  :: (Monad m, PLC.Closed uni, PLC.Everywhere uni SimpleShow
...
runPass logger dumper checkConditions = runPass'
  where
    ...
    runPass' (Pass pt mainPass pre post) t = do
      ...
      pure t'
    -- runPass' (PassMeta p) t = do
    --   x <- runPass' p t
    --   return x
    --   -- runPass' (runWriterT f (metaHandler . show)) t
    runPass' (CompoundPass p1 p2) t = do
      t' <- runPass' p1 t
      runPass' p2 t'
    runPass' (NamedPass n pass) t = do
      lift $ logger $ n ++ ": running pass"
      t' <- runPass' pass t
      lift $ logger $ n ++ ": finished pass"
      pure t'
    runPass' NoOpPass t = pure t
